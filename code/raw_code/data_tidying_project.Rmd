...
The provided code chunks below indicate the process used to download data from the web as a dataframe accessible within RStudio Cloud and the response to the process.
...
This code copies the dataframes as objects saved within the project environment, which is helpful in case the links for the data become broken or otherwise inaccessible in the future.

```{r save-data, warning=FALSE} 
## use saveRDS() to save each object as a .rds file 
saveRDS(df1, "df_departments.rds")
saveRDS(df2, "df_retail.rds")
```

### Wrangle Data

The code chunks below outline the process of shaping and trimming the data to be in a consistent layout and merge them into the same data frame.
...
df_total <- merge(df_department, df_retail, all = TRUE)
```

### Plot Data

At this point, I am not yet familiar with plotting through R. The provided code below generates three distinct plots.
